...
class CMakeBuild(CMakePackage):
            ...
            with binary_library(self.name, target) as install_dir:
                for path in (self.build_path / target).iterdir():
                    if path.suffix in [".lib", ".dll", ".pdb"]:
                        print(
                            f"{PRINT_HEADER}Copying {path} to {install_dir / path.name}"
                        )
    ...
    def copy_headers(self):
        with header_library(self.name) as build_path:
            shutil.copytree(
                self.build_path / "include", build_path.parent, dirs_exist_ok=True
            )
    ...
    def install(self):
        ...
        if (self.install_path / GIT_CACHE_NAME).exists():
            ...
            if existing_hash == self.commit_hash:
                print(
                    f"{PRINT_HEADER}{self.install_path} is up to date, nothing to do."
                )
                return
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            shutil.copytree(
                self.temp_dir, self.install_path, dirs_exist_ok=True, ignore=self.ignore
            )
...
class FmtLib(GitDependency, CMakePackage):
    ...
    def __init__(self):
        CMakePackage.__init__(
            ...
            name=self.DEPENDENCY_NAME,
            build_path=INSTALL_DIRECTORY / self.DEPENDENCY_NAME,
            options={"FMT_INSTALL": False, "FMT_TEST": False, "FMT_DOC": False},
        )
...
class Imgui(GitDependency, BuildSystem):
    ...
    def ignore(self, directory: str, contents: Sequence[str]) -> Sequence[str]:
        ...
        return super().ignore(
            directory, [c for c in contents if c not in ["imconfig.h"]]
        )
...
class Vulkan(GitDependency, BuildSystem):
    ...
    def build(self):
        with header_library(self.DEPENDENCY_NAME) as build_path:
            shutil.copytree(
                self.install_path / "include", build_path, dirs_exist_ok=True
            )
...
class RobinMap(GitDependency, BuildSystem):
    ...
    def build(self):
        with header_library(self.DEPENDENCY_NAME) as build_path:
            shutil.copytree(
                self.install_path / "include" / "tsl", build_path, dirs_exist_ok=True
            )
...
class Tracy(GitDependency, CMakePackage):
    ...
    def __init__(self):
        CMakePackage.__init__(
            ...
            name=self.DEPENDENCY_NAME,
            build_path=INSTALL_DIRECTORY / self.DEPENDENCY_NAME,
            options={"TRACY_ENABLE": True, "TRACY_ON_DEMAND": True},
        )
    ...
    def copy_headers(self):
        with header_library(self.DEPENDENCY_NAME) as build_path:
            shutil.copytree(
                self.install_path / "public", build_path, dirs_exist_ok=True
            )
