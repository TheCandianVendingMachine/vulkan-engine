...
auto ENGINE_NS::blit_image(VkCommandBuffer cmd, VkImage from, VkImage to, VkExtent2D src_size, VkExtent2D dst_size) -> void {
    ...
    blit_region.srcOffsets[1].x = static_cast<std::int32_t>(src_size.width);
    blit_region.srcOffsets[1].y = static_cast<std::int32_t>(src_size.height);
    blit_region.srcOffsets[1].z = 1;

    blit_region.dstOffsets[1].x = static_cast<std::int32_t>(dst_size.width);
    blit_region.dstOffsets[1].y = static_cast<std::int32_t>(dst_size.height);
