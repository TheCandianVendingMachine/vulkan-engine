    ...
    */
    if (rotation.m22 < 0.0) {
        if (rotation.m00 > rotation.m11) {
            ...
            auto s_inv = 1.0 / s;
            values_    = {(rotation.m12 - rotation.m21) * s_inv, 0.25 * s, (rotation.m01 + rotation.m10) * s_inv,
            ...
            auto s_inv = 1.0 / s;
            values_    = {(rotation.m20 - rotation.m02) * s_inv, (rotation.m01 + rotation.m10) * s_inv, 0.25 * s,
        ...
        if (rotation.m00 < -rotation.m11) {
            ...
            auto s_inv = 1.0 / s;
            values_ = {(rotation.m01 - rotation.m10) * s_inv, (rotation.m20 + rotation.m02) * s_inv, (rotation.m12 + rotation.m21) * s_inv,
                       0.25 * s};
            ...
            auto s     = 2.0 * std::sqrt(trace);
            auto s_inv = 1.0 / s;
            values_    = {0.25 * s, (rotation.m12 - rotation.m21) * s_inv, (rotation.m20 - rotation.m02) * s_inv,
    ...
    */
    if (transformation.m22 < 0.0) {
        if (transformation.m00 > transformation.m11) {
            ...
            auto s_inv = 1.0 / s;
            values_    = {(transformation.m12 - transformation.m21) * s_inv, 0.25 * s, (transformation.m01 + transformation.m10) * s_inv,
            ...
            auto s_inv = 1.0 / s;
            values_    = {(transformation.m20 - transformation.m02) * s_inv, (transformation.m01 + transformation.m10) * s_inv, 0.25 * s,
        ...
        if (transformation.m00 < -transformation.m11) {
            ...
            auto s_inv = 1.0 / s;
            values_    = {(transformation.m01 - transformation.m10) * s_inv, (transformation.m20 + transformation.m02) * s_inv,
                          (transformation.m12 + transformation.m21) * s_inv, 0.25 * s};
            if (trace == 1.0 && (values_.x == 0.0 && values_.y == 0.0 && values_.z == 0.0)) [[unlikely]] {
                values_.w = 1.0;
            }
        } else {
            ...
            auto s     = 2.0 * std::sqrt(trace);
            auto s_inv = 1.0 / s;
            values_    = {0.25 * s, (transformation.m12 - transformation.m21) * s_inv, (transformation.m20 - transformation.m02) * s_inv,
...
auto ENGINE_NS::Quaternion::as_rotation_matrix() const -> ::linalg::Matrix3<double> {
    return ::linalg::Matrix3<double>{
      {1.0 - 2.0 * values_.y * values_.y - 2.0 * values_.z * values_.z, 2.0 * values_.x * values_.y - 2.0 * values_.w * values_.z,
       2.0 * values_.x * values_.z + 2.0 * values_.w * values_.y      },
      {2.0 * values_.x * values_.y + 2.0 * values_.w * values_.y,       1.0 - 2.0 * values_.x * values_.x - 2.0 * values_.z * values_.z,
       2.0 * values_.y * values_.z - 2.0 * values_.w * values_.x      },
      {2.0 * values_.x * values_.z - 2.0 * values_.w * values_.y,       2.0 * values_.y * values_.z + 2.0 * values_.w * values_.x,
