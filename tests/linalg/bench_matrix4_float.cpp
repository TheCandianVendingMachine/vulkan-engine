...
namespace bench_matrix3 {
    ...
    auto random_matrix_lower_triangular(Random &rng) -> linalg::Matrix4<float> {
        return linalg::Matrix4<float>{
            rng.range(range()), 0.f, 0.f, 0.f,
            rng.range(range()), rng.range(range()), 0.f, 0.f,
            rng.range(range()), rng.range(range()), rng.range(range()), 0.f,
            rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()),
        };
    }

    auto random_matrix_upper_triangular(Random &rng) -> linalg::Matrix4<float> {
        return linalg::Matrix4<float>{
            rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()),
            0.f,                rng.range(range()), rng.range(range()), rng.range(range()),
            0.f,                0.f,                rng.range(range()), rng.range(range()),
            0.f,                0.f,                0.f,                rng.range(range()),
        };
    }
