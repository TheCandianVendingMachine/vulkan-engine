...
auto random_vector(engine::Random& rng) -> linalg::Vector2<float> {
...
auto random_matrix(engine::Random& rng) -> linalg::Matrix2<float> {
...
auto random_matrix_lower_triangular(engine::Random& rng) -> linalg::Matrix2<float> {
    return linalg::Matrix2<float>{
      rng.range(range()),
      0.f,
      rng.range(range()),
...
auto random_matrix_upper_triangular(engine::Random& rng) -> linalg::Matrix2<float> {
    return linalg::Matrix2<float>{
      rng.range(range()),
      rng.range(range()),
      0.f,
