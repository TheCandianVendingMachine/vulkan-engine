...
auto random_vector(Random& rng) -> linalg::Vector2<float> { return linalg::Vector2<float>{rng.range(range()), rng.range(range())}; }

auto random_matrix(Random& rng) -> linalg::Matrix2<float> {
    return linalg::Matrix2<float>{rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range())};
}
...
TEST_CASE("Matrix2LU::from [Matrix2<float>]", "[Matrix2]") {
    SECTION("Single") {
        linalg::Matrix2<float> A(2.f, 5.f, 1.f, 3.f);
...
TEST_CASE("blas2::solve_lower_triangular [Matrix2<float>]", "[blas2][Matrix2]") {
    SECTION("Single") {
        linalg::Matrix2<float> A(2.f, 0.f, 1.f, 3.f);
        linalg::Vector2<float> b(11.f, 6.f);
...
TEST_CASE("blas2::solve_upper_triangular [Matrix2<float>]", "[blas2][Matrix2]") {
    SECTION("Single") {
        linalg::Matrix2<float> A(2.f, 5.f, 0.f, 3.f);
        linalg::Vector2<float> b(11.f, 6.f);
