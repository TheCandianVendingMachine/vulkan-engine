...
namespace bench_matrix3 {
    ...
    auto random_vector(engine::Random& rng) -> linalg::Vector3<float> {
    ...
    auto random_matrix(engine::Random& rng) -> linalg::Matrix3<float> {
        return linalg::Matrix3<float>{rng.range(range()),
                                      rng.range(range()),
                                      rng.range(range()),
                                      rng.range(range()),
                                      rng.range(range()),
    ...
    auto random_matrix_lower_triangular(engine::Random& rng) -> linalg::Matrix3<float> {
    ...
    auto random_matrix_upper_triangular(engine::Random& rng) -> linalg::Matrix3<float> {
        return linalg::Matrix3<float>{
          rng.range(range()),
          rng.range(range()),
          rng.range(range()),
