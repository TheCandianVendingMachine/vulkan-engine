...
auto random_matrix(Random& rng) -> linalg::Matrix3<float> {
    return linalg::Matrix3<float>{rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()),
                                  rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range())};
}

auto random_matrix_lower_triangular(Random& rng) -> linalg::Matrix3<float> {
    return linalg::Matrix3<float>{
        rng.range(range()), 0.f, 0.f, rng.range(range()), rng.range(range()), 0.f, rng.range(range()), rng.range(range()),
        rng.range(range()),
    };
}

auto random_matrix_upper_triangular(Random& rng) -> linalg::Matrix3<float> {
    return linalg::Matrix3<float>{
        rng.range(range()), rng.range(range()), rng.range(range()), 0.f, rng.range(range()), rng.range(range()), 0.f, 0.f,
        rng.range(range()),
    };
}

TEST_CASE("blas2 - bench [Matrix3<float>]", "[blas2][Matrix3][bench]") {
    auto rng = Random();
    BENCHMARK_ADVANCED("linalg::lu_decomposition - random")(Catch::Benchmark::Chronometer meter) {
        auto A = random_matrix(rng);
        meter.measure([&] { return linalg::Matrix3LU<float>::from(A); });
    };
