...
namespace bench_matrix3 {
    ...
    auto random_matrix_lower_triangular(Random &rng) -> linalg::Matrix4<double> {
        return linalg::Matrix4<double>{
            rng.range(range()), 0.0, 0.0, 0.0,
            rng.range(range()), rng.range(range()), 0.0, 0.0,
            rng.range(range()), rng.range(range()), rng.range(range()), 0.0,
            rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()),
        };
    }
    ...
    auto random_matrix_upper_triangular(Random &rng) -> linalg::Matrix4<double> {
        return linalg::Matrix4<double>{
            rng.range(range()), rng.range(range()), rng.range(range()), rng.range(range()),
            0.0,                rng.range(range()), rng.range(range()), rng.range(range()),
            0.0,                0.0,                rng.range(range()), rng.range(range()),
            0.0,                0.0,                0.0,                rng.range(range()),
        };
    }
